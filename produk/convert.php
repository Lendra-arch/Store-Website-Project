...
if (isset($_SESSION['temp_data'])) {
    unset($_SESSION['temp_data']); // Hapus data sementara setelah digunakan
}
    ...
    <div class="container">
        ...
        <form id="convert-form" method="POST" action="convert.php" class="form-section">
            ...
            <div class="card">
                ...
                <input type="hidden" name="convert_category" id="convert_category" 
                       value="<?= $temp_data['convert_category'] ?? '' ?>">
                <input type="hidden" name="convert_price" id="convert_price" 
                       value="<?= $temp_data['convert_price'] ?? 0 ?>">
            </div>

            <!-- Nominal -->
            <div class="card">
                <h3>Masukkan Nominal</h3>
                <input type="number" name="amount" id="amount" placeholder="Nominal" 
                       value="<?= isset($temp_data['amount']) ? htmlspecialchars($temp_data['amount']) : '' ?>" required oninput="updateConvertedAmount()">
